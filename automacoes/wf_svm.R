...
wf_svm <- function(test_size, datasets){
  ...
  #ranges <- list(kernel=c('radial','linear'), epsilon = c(0.01, 0.1, 0.2), cost = c(0.1, 1, 10, 50, 100))
  ranges <- list(kernel=c('radial','linear'), epsilon = c(0.01, 0.1), cost = c(0.1, 1))
  ...
  for (ds in datasets) {
    ...
    for (ts in colnames(df)) {
      ...
      filename <- sprintf('%s/%s_%s', sub('-.*', '', 'output'), ts, 'svm')
      print(filename)
      ...
      for (i in 1:length(cases)) {
        ...
        print(name)
        tryCatch({
          novo_resultado <- run_ml(df[[ts]], name, ts_svm(), test_size=test_size, params=cases[[i]], stgy=list(sa=TRUE, ro=FALSE, image=FALSE))
      ...
      if (!is.null(results2) && nrow(results2) > 0) {
        ...
        for (j in 1:nrow(melhor_modelo)) {
          ...
          ranges2 <- list(kernel = cases_melhor_3$params$ranges$kernel,
                          epsilon = as.numeric(cases_melhor_3$params$ranges$epsilon),
                          cost = as.numeric(cases_melhor_3$params$ranges$cost))
          
          params2 <- list(sw_size=sw_size2, preprocess=preprocess2, augment=augment2, ranges=ranges2)
          
          name2 <- get_names(filename, params2)
          
          run_ml(df[[ts]], name2, ts_svm(), test_size=test_size, params=params2, stgy=list(sa=TRUE, ro=FALSE, image=TRUE), save_model=TRUE)
        }
      }
