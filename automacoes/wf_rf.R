...
for (ds in datasets) {
  ...
  for (ts in colnames(df)) {
    filename <- sprintf('%s/%s_%s', sub('-.*', '', ds), ts, 'rfr')
    print(filename)
    #run_ml(df[[ts]], filename, ts_rf(), test_size=test_size, params=params)
    ...
    for (i in 1:length(cases)) {
      ...
      print(name)
      tryCatch({
        novo_resultado <- run_ml(df[[ts]], name, ts_rf(), test_size=test_size, params=cases[[i]], stgy=list(sa=TRUE, ro=FALSE))
